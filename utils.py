...
def find_repeats(seq, flank_size, repeat_units):
    ...
    for ru in repeat_units:
        ...
        while base < bases-flank_size: #and base not in exclude:
            ...
            scores = []
            while ( (test_pos ) < (bases-flank_size) )  and  score > FAIL_SCORE and \
...
def phased(msi_instance, sites, bam_path):
    ...
    for site in sites:
        ...
        reads = [
            ...
            read.mapping_quality >= msi_instance.mapping_quality
            ]
        if len(reads) > msi_instance.min_coverage:
            for read in reads:
                read_sequence = read.seq
                # read_sequence = read.query_alignment_sequence
                reps = find_repeats(
                    read_sequence,
                    msi_instance.flank_size,
                    msi_instance.repeat_units
                )
                if len(reps) > 0:
                    ...
                    for microsatellite in reps:
                        ...
                        ini = start_read + rs  #
                        idx2 = binary_search(
                            msi_instance.reference_set_ini_end_dict[chr],
                            str(ini + 1)
                        )

                        if idx2 == -1:
                            continue
                        refset_now = msi_instance.reference_set_dict[chr][idx2]
                        diff_ref = int(refset_now[2]) - int(
                            refset_now[1]) + 1
                        fasta_file = pysam.FastaFile(
                            filename=msi_instance.fasta_dict[chr]
                        )
                        flank_right_ref = fasta_file.fetch(
                            "chr" + str(site[0]), ini + diff_ref,
                            ini + diff_ref + msi_instance.flank_size).upper()
                        flank_left_ref = fasta_file.fetch(
                            "chr" + str(site[0]), ini -
                                                  msi_instance.flank_size,
                            ini).upper()
                        posfl = (start_read + rs - msi_instance.flank_size)
                        if posfl >= start_read:
                            flank_left = read_sequence[
                                         rs - msi_instance.flank_size:rs]
                            mismatches_left = sum(a != b for a, b in
                                                  zip(flank_left,
                                                      flank_left_ref))
                        else:
                            mismatches_left = 10000
                        posflr = start_read + re + msi_instance.flank_size
                        if posflr <= end_read:
                        ...
                        if mismatches <= msi_instance.tolerated_mismatches:
...
def unphased(msi_instance, sites, bam_path):
    bamfile = pysam.AlignmentFile(bam_path, "rb")
    dict_out = {};
    visited_reads = []
    for site in sites:
        ...
        reads = [read for read in reads if
                 read.is_proper_pair and read.is_duplicate == False and read.mapping_quality >= msi_instance.mapping_quality]
        if len(reads) > msi_instance.min_coverage:
            for read in reads:
                start_read = read.reference_start;
                end_read = read.reference_end
                read_sequence = read.seq
                reps = find_repeats_target(read_sequence, msi_instance.flank_size, ru)
                if len(reps) > 0:
                    aligned_pos = read.get_reference_positions(
                        full_length=True)
                    try:
